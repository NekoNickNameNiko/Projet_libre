...
class jessica(operateur_base):
    def __init__(self,x,y):
        super().__init__(x,y,70,23,"Jessica")
        self.jessica_imgs_start = Jessica_imgs_start[:]
        self.operateur_imgs_idle = Jessica_imgs_idle[:]
        self.jessica_imgs_front_attack = Jessica_imgs_front_attack[:]
        self.jessica_imgs_back_attack = Jessica_imgs_back_attack[:]
    ...
    def deployer(self,fenetre):
        ...
        self.start = self.jessica_imgs_start[self.animation_count_start//2]

        if self.deployed == False and self.animation_count_start >= len(self.jessica_imgs_start)*2-1:
            self.animation_count_start = 0
            self.deployed = True
        else:
            if self.animation_count_start >= len(self.jessica_imgs_start)*2-1:
                self.animation_count_start = 0
    ...
    def draw_attack(self,fenetre, enemy):
        ...
        if en_x < self.x and not self.left:
            ...
            for x, img in enumerate(self.jessica_imgs_front_attack):
                self.jessica_imgs_front_attack[x] = pygame.transform.flip(img, True, False)
            for x, img in enumerate(self.jessica_imgs_back_attack):
                self.jessica_imgs_back_attack[x] = pygame.transform.flip(img, True, False)
        ...
        if self.animation_count >= len(self.jessica_imgs_front_attack)*2:
            self.animation_count = 0

        #initialise les images utilisé
        self.img_front = self.jessica_imgs_front_attack[self.animation_count//2]
        self.img_back  = self.jessica_imgs_back_attack[self.animation_count//2]

        self.fire(self.animation_count,18)
