...
class steward(operateur_base):
    def __init__(self,x,y):
        super().__init__(x,y,70,23,"Steward")
        self.steward_imgs_start = steward_imgs_start[:]
        self.operateur_imgs_idle = steward_imgs_idle[:]
        self.steward_imgs_front_attack = steward_imgs_front_attack[:]
        self.steward_imgs_back_attack = steward_imgs_back_attack[:]
    ...
    def deployer(self,fenetre):
        ...
        self.start = self.steward_imgs_start[self.animation_count_start//2]

        if self.deployed == False and self.animation_count_start >= len(self.steward_imgs_start)*2-1:
            self.animation_count_start = 0
            self.deployed = True
        else:
            if self.animation_count_start >= len(self.steward_imgs_start)*2-1:
                self.animation_count_start = 0
    ...
    def draw_attack(self,fenetre, enemy):
        ...
        if en_x < self.x and not self.left:
            ...
            for x, img in enumerate(self.steward_imgs_front_attack):
                self.steward_imgs_front_attack[x] = pygame.transform.flip(img, True, False)
            for x, img in enumerate(self.steward_imgs_back_attack):
                self.steward_imgs_back_attack[x] = pygame.transform.flip(img, True, False)
        ...
        if self.animation_count >= len(self.steward_imgs_front_attack)*4:
            self.animation_count = 0

        #initialise les images utilisé
        self.img_front = self.steward_imgs_front_attack[self.animation_count//4]
        self.img_back  = self.steward_imgs_back_attack[self.animation_count//4]
